...
export default function SettingsPanel({ isOpen, onClose }: SettingsPanelProps) {
  ...
  const [accentColor, setAccentColor] = useState<string>(() => {
    if (typeof localStorage !== 'undefined') {
      return localStorage.getItem('accent-color') || '#FA5D19';
    }
    return '#FA5D19';
  });
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          {/* Header - Fixed */}
          <div className="p-24 border-b border-gray-200 dark:border-gray-900 flex-shrink-0">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-title-h4 text-accent-black dark:text-white">Configuration Status</h2>
                <p className="text-body-small text-black-alpha-48 dark:text-gray-400 mt-4">
          ...
          <div className="p-24 overflow-y-auto flex-1">
            {loading ? (
              <div className="text-center py-32">
                <div className="inline-block w-32 h-32 border-4 border-heat-100 dark:border-heat-100 border-t-transparent rounded-full animate-spin"></div>
                <p className="text-body-small text-black-alpha-48 dark:text-gray-400 mt-12">Loading configuration...</p>
              </div>
            ) : (
              <div className="space-y-20">
                ...
                <div>
                  <div className="flex items-center justify-between mb-12">
                    <h3 className="text-label-large font-medium text-accent-black dark:text-white">LLM Providers</h3>
                    <button
                  ...
                  <div className="space-y-8">
                    {['anthropic', 'openai', 'groq'].map(provider => {
                      ...
                      return (
                        <div key={provider} className="p-12 bg-background-base dark:bg-[#1c1d24] rounded-8">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-8">
                              <StatusIcon configured={!!(providerKey || hasEnvKey)} />
                              <div>
                                <p className="text-body-small text-accent-black dark:text-white font-medium capitalize">{provider}</p>
                                {providerKey ? (
                                  <p className="text-xs text-black-alpha-48 dark:text-gray-400">
                                    Key: {providerKey.keyPrefix} {providerKey.label && `(${providerKey.label})`}
                                  </p>
                                ) : hasEnvKey ? (
                                  <p className="text-xs text-black-alpha-48 dark:text-gray-400">Using environment variable</p>
                                ) : (
                                  <p className="text-xs text-black-alpha-48 dark:text-gray-400">Not configured</p>
                                )}
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-8">
                              {providerKey && (
                                <>
                                  <button
                                    ...
                                    title="Remove key"
                                  >
                                    <Trash2 className="w-14 h-14 text-black-alpha-48 hover:text-accent-black dark:text-white" />
                                  </button>
                                </>
                              )}
                              <button
                              ...
                              >
                                {providerKey ? (
                                  <Edit className="w-14 h-14 text-black-alpha-72 hover:text-accent-black dark:text-white" />
                                ) : (
                                  <Plus className="w-14 h-14 text-black-alpha-72 hover:text-accent-black dark:text-white" />
                                )}
                              </button>
                            </div>
                          </div>
                  ...
                  <div className="mt-8 p-12 bg-heat-4 dark:bg-heat-100/10 border border-heat-100 dark:border-heat-100/30 rounded-8">
                    <p className="text-xs text-black-alpha-64 dark:text-gray-400">
                ...
                {/* Appearance */}
                <div>
                  <h3 className="text-label-large font-medium text-accent-black dark:text-white mb-12">Appearance</h3>
                  <div className="p-12 bg-background-base dark:bg-[#1c1d24] rounded-8">
                    <label className="text-body-small text-accent-black dark:text-white font-medium mb-8 block">Accent Color</label>
                    <div className="flex gap-8 flex-wrap">
                      {[
                        { name: 'Orange', value: '#FA5D19', hover: '#eb5617' },
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            document.documentElement.style.setProperty('--accent-color-hover', color.hover);
                            toast.success(`Accent color changed to ${color.name}`);
                          }}
                          className={`flex-1 px-12 py-8 rounded-8 text-xs font-medium transition-all border-2 ${
                            accentColor === color.value
                              ? 'border-white dark:border-black-alpha-48 shadow-lg'
                              : 'border-transparent'
                          }`}
                ...
                {/* Integrations */}
                <div>
                  <h3 className="text-label-large font-medium text-accent-black dark:text-white mb-12">Integrations</h3>
                  <div className="grid grid-cols-1 gap-8">
                    <div className="flex items-center gap-8 p-10 bg-background-base dark:bg-[#1c1d24] rounded-8">
                      <StatusIcon configured={serverConfig?.firecrawlConfigured || false} />
                      <div className="min-w-0">
                        <p className="text-body-small text-accent-black dark:text-white font-medium truncate">Firecrawl</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {/* API Keys */}
                <div>
                  <h3 className="text-label-large font-medium text-accent-black dark:text-white mb-12">Your API Keys</h3>

                  {/* Show generated key once */}
                  {generatedKey && (
                    <div className="p-16 bg-heat-4 dark:bg-heat-100/10 border border-heat-100 dark:border-heat-100/30 rounded-8 mb-12">
                      <div className="flex items-start gap-12 mb-12">
                        <AlertCircle className="w-20 h-20 text-heat-100 flex-shrink-0 mt-2" />
                        <div className="flex-1">
                          <p className="text-body-medium text-accent-black dark:text-white font-medium mb-4">
                            Save this key now!
                          </p>
                          <p className="text-body-small text-black-alpha-64 dark:text-gray-300 mb-8">
                            You won't be able to see it again.
                          </p>
                          <div className="flex items-center gap-8">
                            <code className="flex-1 px-12 py-8 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-900 rounded-8 text-xs font-mono text-accent-black dark:text-white">
                              {generatedKey}
                            </code>
                            <button
                              onClick={() => {
                      ...
                      <button
                        ...
                        className="text-body-small text-black-alpha-64 dark:text-gray-300 hover:text-accent-black dark:hover:text-white"
                      >
                        I've saved it
                      </button>
                    </div>
                  ...
                  <div className="flex gap-8 mb-12">
                    <input
                      ...
                      className="flex-1 px-12 py-8 bg-background-base dark:bg-[#1c1d24] rounded-8 text-body-small text-accent-black dark:text-white placeholder:text-black-alpha-32"
                    />
                    <button
                      onClick={async () => {
                        if (!newKeyName.trim()) {
                  ...
                  <div className="space-y-8">
                    {apiKeys?.map((key) => (
                      <div
                        key={key._id}
                        className="flex items-center justify-between p-12 bg-background-base dark:bg-[#1c1d24] rounded-8"
                      >
                        <div className="flex-1 min-w-0">
                          <p className="text-body-small text-accent-black dark:text-white font-medium">{key.name}</p>
                          <code className="text-xs text-black-alpha-48 dark:text-gray-400 font-mono">{key.keyPrefix}</code>
                        ...
                        <button
                          ...
                          title="Revoke key"
                        >
                          <Trash2 className="w-16 h-16 text-black-alpha-72 hover:text-accent-black dark:text-white" />
                        </button>
                      </div>
                    ))}

                    {(!apiKeys || apiKeys.length === 0) && !generatedKey && (
                      <p className="text-body-small text-black-alpha-48 dark:text-gray-400 text-center py-16">
                ...
                <div>
                  <div className="flex items-center justify-between mb-12">
                    <h3 className="text-label-large font-medium text-accent-black dark:text-white">MCP Registry</h3>
                    <div className="flex gap-8">
                      <button
                        onClick={() => setShowPasteConfigModal(true)}
                        className="px-12 py-6 bg-black-alpha-4 hover:bg-black-alpha-8 text-accent-black dark:text-white rounded-8 text-body-small font-medium transition-all active:scale-[0.98] flex items-center gap-6 border border-gray-200 dark:border-gray-900"
                      >
                  ...
                  <div className="space-y-8">
                    ...
                    {(!mcpServers || mcpServers.length === 0) && (
                      <div className="text-center py-32">
                        <Plug className="w-48 h-48 text-black-alpha-16 dark:text-gray-700 mx-auto mb-12" />
                        <p className="text-body-small text-black-alpha-48 dark:text-gray-400 mb-16">
                ...
                {/* Info Box */}
                <div className="p-16 bg-heat-4 dark:bg-heat-100/10 border border-heat-100 dark:border-heat-100/30 rounded-8">
                  <div className="flex items-start gap-12">
                    ...
                    <div>
                      ...
                      <p className="text-body-small text-black-alpha-64 dark:text-gray-300">
                        LLM & Integration keys are set in <code className="px-4 py-2 bg-white dark:bg-[#0d0d0d] rounded text-xs font-mono text-heat-100">.env.local</code>.
          ...
          {/* Footer - Fixed */}
          <div className="p-20 border-t border-gray-200 dark:border-gray-900 bg-background-base dark:bg-[#1c1d24] rounded-b-16 flex-shrink-0">
            <button
              onClick={onClose}
...
function MCPCard({
  ...
  const getStatusColor = () => {
    switch (server.connectionStatus) {
      ...
      case 'error': return 'text-accent-black dark:text-white';
  ...
  const getAuthIcon = () => {
    switch (server.authType) {
      case 'api-key': return <Key className="w-12 h-12 text-black-alpha-72" />;
      case 'bearer': return <Shield className="w-12 h-12 text-black-alpha-72" />;
  ...
  return (
    <div className={`bg-background-base dark:bg-[#1c1d24] rounded-8 border ${server.enabled ? 'border-gray-200 dark:border-gray-900' : 'border-black-alpha-8 dark:border-gray-900 opacity-60'}`}>
      {/* Header */}
      <div className="p-12 flex items-center gap-8">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-8">
            <p className="text-body-small text-accent-black dark:text-white font-medium">{server.name}</p>
            {server.isOfficial && (
          ...
          {server.description && (
            <p className="text-xs text-black-alpha-48 dark:text-gray-400 mt-2">{server.description}</p>
          )}
      ...
      {/* Expanded Content */}
      {isExpanded && (
        <div className="px-12 pb-12 border-t border-gray-200 dark:border-gray-900">
          <div className="pt-12 space-y-12">
            {/* URL & Auth */}
            <div className="grid grid-cols-2 gap-12">
              <div>
                <p className="text-xs text-black-alpha-48 dark:text-gray-400 mb-4">URL</p>
                <code className="text-xs font-mono text-accent-black dark:text-white bg-black-alpha-4 dark:bg-gray-900 px-8 py-4 rounded-4 block truncate">
                  {server.url}
                </code>
              </div>
              <div>
                <p className="text-xs text-black-alpha-48 dark:text-gray-400 mb-4">Authentication</p>
                <div className="flex items-center gap-4">
                  <span className="text-xs text-accent-black dark:text-white capitalize">
                    {server.authType === 'oauth-coming-soon' ? 'OAuth (Coming Soon)' : server.authType.replace('-', ' ')}
                  </span>
                  {server.accessToken && <span className="text-xs text-black-alpha-32 dark:text-gray-600">•••••••</span>}
                </div>
              </div>
            </div>
            ...
            {server.tools && server.tools.length > 0 && (
              <div>
                <p className="text-xs text-black-alpha-48 dark:text-gray-400 mb-4">Available Tools ({server.tools.length})</p>
                <div className="flex flex-wrap gap-4">
                  {server.tools.map((tool) => (
                    <span key={tool} className="px-8 py-4 bg-black-alpha-4 dark:bg-gray-900 text-xs text-accent-black dark:text-white rounded-4">
                      {tool}
                    </span>
                  ))}
                </div>
            ...
            <div className="flex items-center gap-12 text-xs">
              {server.lastTested && (
                <span className="text-black-alpha-48 dark:text-gray-400">
                  Last tested: {new Date(server.lastTested).toLocaleString()}
                </span>
              )}
              {server.lastError && (
                <span className="text-accent-black dark:text-red-400">
            ...
            <div className="flex items-center gap-8 pt-8">
              <button
                onClick={onTest}
                disabled={isTesting}
                className="px-12 py-6 bg-black-alpha-4 hover:bg-black-alpha-8 text-accent-black dark:text-white rounded-8 text-body-small transition-all flex items-center gap-6 disabled:opacity-50"
              >
              ...
              <button
                onClick={onEdit}
                className="px-12 py-6 bg-black-alpha-4 hover:bg-black-alpha-8 text-accent-black dark:text-white rounded-8 text-body-small transition-all flex items-center gap-6"
              >
  ...
  return (
    ...
    >
      <motion.div
        initial={{ scale: 0.95 }}
        animate={{ scale: 1 }}
        className="bg-card rounded-16 shadow-2xl max-w-md w-full mx-20 max-h-[85vh] overflow-hidden flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-20 border-b border-gray-200 dark:border-gray-900 flex-shrink-0">
          <h3 className="text-label-large text-accent-black dark:text-white font-medium">
        ...
        <div className="p-24 space-y-16 overflow-y-auto flex-1">
          <div>
            <label className="text-body-small text-black-alpha-64 dark:text-gray-400 mb-4 block">Name</label>
            <input
              type="text"
              value={formData.name}
              onChange={(e) => setFormData({ ...formData, name: e.target.value })}
              placeholder="e.g., Firecrawl"
              className="w-full px-12 py-8 bg-background-base dark:bg-[#1c1d24] rounded-8 text-body-small text-accent-black dark:text-white"
            />
          </div>

          <div>
            <label className="text-body-small text-black-alpha-64 dark:text-gray-400 mb-4 block">URL</label>
            <input
              type="text"
              value={formData.url}
              onChange={(e) => setFormData({ ...formData, url: e.target.value })}
              placeholder="https://mcp.example.com"
              className="w-full px-12 py-8 bg-background-base dark:bg-[#1c1d24] rounded-8 text-body-small text-accent-black dark:text-white font-mono"
            />
          </div>

          <div>
            <label className="text-body-small text-black-alpha-64 dark:text-gray-400 mb-4 block">Description (optional)</label>
            <input
              type="text"
          ...
          <div>
            <label className="text-body-small text-black-alpha-64 dark:text-gray-400 mb-4 block">Category</label>
            <select
          ...
          <div>
            <label className="text-body-small text-black-alpha-64 dark:text-gray-400 mb-4 block">Authentication</label>
            <select
          ...
          {(formData.authType === 'api-key' || formData.authType === 'bearer') && (
            <div>
              <label className="text-body-small text-black-alpha-64 dark:text-gray-400 mb-4 block">
          ...
          <div>
            <button
              ...
              className="w-full px-16 py-10 bg-black-alpha-4 hover:bg-black-alpha-8 text-accent-black dark:text-white rounded-8 text-body-small font-medium transition-all flex items-center justify-center gap-6 disabled:opacity-50"
            >
              {isTesting ? (
                <>
              ...
              <div className="p-12 bg-heat-4 dark:bg-heat-100/10 rounded-8 border border-heat-100 dark:border-heat-100/30">
                <div className="flex flex-wrap gap-4">
                  {discoveredTools.map((tool) => (
                    <span key={tool} className="px-6 py-2 bg-white dark:bg-gray-800 text-heat-100 rounded-4 text-xs font-medium border border-heat-100 dark:border-heat-100/50">
                      {tool}
                    </span>
                  ))}
                </div>
        ...
        <div className="p-20 border-t border-gray-200 dark:border-gray-900 flex gap-8 flex-shrink-0">
          <button
            onClick={onClose}
            className="flex-1 px-20 py-12 bg-black-alpha-4 hover:bg-black-alpha-8 text-accent-black dark:text-white rounded-8 text-body-medium font-medium transition-all"
          >
            Cancel
          </button>
          <button
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="p-24 space-y-16 overflow-y-auto flex-1">
          <div>
            <label className="text-body-small text-black-alpha-64 dark:text-gray-400 mb-4 block">Provider</label>
            <select
              value={formData.provider}
              onChange={(e) => setFormData({ ...formData, provider: e.target.value as 'anthropic' | 'openai' | 'groq' })}
              disabled={!!selectedProvider}
              className="w-full px-12 py-8 bg-background-base dark:bg-[#1c1d24] rounded-8 text-body-small text-accent-black dark:text-white capitalize"
            >
          ...
          <div>
            <label className="text-body-small text-black-alpha-64 dark:text-gray-400 mb-4 block">API Key</label>
            <div className="relative">
              <input
                ...
                className="w-full pr-32 px-12 py-8 bg-background-base dark:bg-[#1c1d24] rounded-8 text-body-small text-accent-black dark:text-white font-mono"
              />
              <button
                type="button"
              ...
              >
                {showKey ? (
                  <EyeOff className="w-16 h-16 text-black-alpha-48 dark:text-gray-400" />
                ) : (
                  <Eye className="w-16 h-16 text-black-alpha-48 dark:text-gray-400" />
                )}
              </button>
            </div>
            <a
          ...
          <div>
            <label className="text-body-small text-black-alpha-64 dark:text-gray-400 mb-4 block">Label (optional)</label>
            <input
              type="text"
          ...
          <div className="p-12 bg-heat-4 dark:bg-heat-100/10 border border-heat-100 dark:border-heat-100/30 rounded-8">
            <p className="text-xs text-black-alpha-64 dark:text-gray-300">
        ...
        <div className="p-20 border-t border-gray-200 dark:border-gray-900 flex gap-8 flex-shrink-0">
          <button
            onClick={onClose}
            disabled={isSaving}
            className="flex-1 px-20 py-12 bg-black-alpha-4 hover:bg-black-alpha-8 text-accent-black dark:text-white rounded-8 text-body-medium font-medium transition-all disabled:opacity-50"
          >
            Cancel
          </button>
          <button
